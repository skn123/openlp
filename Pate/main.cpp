...
int main(){
  ...
  //******************************************************************************************
    ...
    cl_mem matrix_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,numCons*numVars * sizeof(cl_float), NULL, &ret);
    cl_mem output_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, numCons*numVars * sizeof(cl_float), NULL, &ret);
	...
	cl_int matrixSize = numCons*numVars;
	ret = clSetKernelArg(kernel, 2, sizeof(cl_int), (void *)&matrixSize);
 
    // Execute the OpenCL kernel on the list
    ret = clEnqueueTask(command_queue, kernel, 0, NULL,NULL);
 
    // Read the memory buffer C on the device to the local variable C
    cl_float *C = (cl_float*)malloc(sizeof(cl_float)*numCons*numVars);
    ret = clEnqueueReadBuffer(command_queue, output_mem_obj, CL_TRUE, 0,numCons*numVars * sizeof(cl_float), C, 0, NULL, NULL);
